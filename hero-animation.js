(function() {
  ...
  function init() {
    ...
    const configs = [
      { rBase: dim * 0.38, home: { x: 0.55, y: 0.40 }, freq: 0.0005, amp: 0.08, phase: 0, strokeAlpha: 0.55, lineWidth: 1.8, cogDir: 1 },
      { rBase: dim * 0.24, home: { x: 0.30, y: 0.60 }, freq: 0.0008, amp: 0.10, phase: 2.1, strokeAlpha: 0.45, lineWidth: 1.4, cogDir: -1 },
      { rBase: dim * 0.14, home: { x: 0.75, y: 0.65 }, freq: 0.0012, amp: 0.12, phase: 4.0, strokeAlpha: 0.40, lineWidth: 1.2, cogDir: 1 },
      { rBase: dim * 0.08, home: { x: 0.40, y: 0.25 }, freq: 0.0018, amp: 0.15, phase: 5.8, strokeAlpha: 0.65, lineWidth: 1.8, cogDir: -1 }
    ];
  ...
  function drawBackground() {
    ...
    for (var i = 0; i < blobs.length; i++) {
      ...
      grad.addColorStop(0.4, 'rgba(' + b.color[0] + ',' + b.color[1] + ',' + b.color[2] + ',0.45)');
    ...
    for (let i = 0; i < circles.length; i++) {
      ...
      if (mouse.smoothX > -1000) {
        ...
        if (dist < range) {
          ...
          c.orbitAngle += 0.012 + proximity * 0.02;
          const orbitRadius = dist * 0.2 * (1 - proximity * 0.5);
          const orbitX = Math.cos(c.orbitAngle) * orbitRadius * proximity;
          const orbitY = Math.sin(c.orbitAngle) * orbitRadius * proximity;

          const pullStrength = proximity * proximity * 0.4;
          targetX += dx * pullStrength + orbitX;
          targetY += dy * pullStrength + orbitY;

          if (speedFactor > 0.3) {
            const pushAngle = Math.atan2(dy, dx);
            const pushDist = speedFactor * proximity * maxDim * 0.04;
            targetX += Math.cos(pushAngle + Math.PI * 0.5 * (i % 2 === 0 ? 1 : -1)) * pushDist;
            targetY += Math.sin(pushAngle + Math.PI * 0.5 * (i % 2 === 0 ? 1 : -1)) * pushDist;
          }

          const targetBoost = proximity * 0.08 + speedFactor * proximity * 0.05;
          c.sizeBoost += (targetBoost - c.sizeBoost) * 0.04;

          c.strokeAlpha += ((c.baseAlpha + proximity * 0.15) - c.strokeAlpha) * 0.05;
        } else {
          c.sizeBoost *= 0.97;
      ...
      c.x += (targetX - c.x) * 0.025;
      c.y += (targetY - c.y) * 0.025;

      const breath = 1 + Math.sin(time * 0.012 + c.breathPhase) * 0.03;
      c.r = c.rBase * breath * (1 + c.sizeBoost);
    }

    // Cog-like collision: tangential force transfer between touching circles
    for (let i = 0; i < circles.length; i++) {
      for (let j = i + 1; j < circles.length; j++) {
        ...
        if (dist < minDist && dist > 0) {
          ...
          // Cog tangential transfer: movement of one rolls the other
          const tangentX = -ny;
          const tangentY = nx;
          const aVelAlongNormal = (a.x - a.homeX) * nx + (a.y - a.homeY) * ny;
          const cogForce = aVelAlongNormal * 0.008;

          b.x += tangentX * cogForce * b.cogDir;
          b.y += tangentY * cogForce * b.cogDir;
          a.x += -tangentX * cogForce * a.cogDir * 0.6;
          a.y += -tangentY * cogForce * a.cogDir * 0.6;
        }
      }
    }
  }
  ...
  function draw() {
    ...
    // Strokes only — no gradient fills
